...
4. Set AI role metadata so `CombatBrain` can score moves in context.

## Create enemy profile

1. Create a `CombatantProfile` resource.
2. Set display fields, base stats or `stat_weights`, timeline marker data, and health bar config. Enemy level scaling uses `stat_weights` to allocate rolled stat budgets.
...
3. Set `min_enemy_count` / `max_enemy_count`, then add `enemy_slots` dictionaries with `combatant_profile_path` pointing at the enemy profile and `position_id` values such as `EnemySlot1`.
...
- Confirm `BattleScene` loads generated enemy instances from `GameManager.get_current_encounter()` and places each enemy at the expected authored slot marker.
